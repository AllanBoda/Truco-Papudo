...
class Game {
  ...
  void iniciarJogo() {
    ...
    definirManilha();
    jogadorAtual =
        baralho.listaJogador.isNotEmpty ? baralho.listaJogador.first : null;
    //CartasNaMesa();
  }

  void escolherCartaDaJogada(int indiceCarta, Jogador jogador,
      {bool pediuTruco = false}) {
    if (indiceCarta >= 0 && indiceCarta < jogador.maoJogador.length) {
      Cartas carta = jogador.maoJogador[indiceCarta];
      baralho.cartasNaMesa
          .add(CartaNaMesa(carta, jogador, trucoPediu: pediuTruco));
  ...
  void definirGanhadorRodada(
      List<Cartas> forcaCartas, List<CartaNaMesa> cartasNaMesa, bool trucou) {
    int posicaoMaisAlta = 0;
    int jogadorVencedor = -1;
    int menorDiferenca = forcaCartas.length;

    while (rodada < 3) {
      if (cartasNaMesa[0].jogador.pontos <= 12 ||
          cartasNaMesa[1].jogador.pontos <= 12) {
        ...
        for (int i = 0; i < cartasNaMesa.length; i++) {
          ...
          if (cartaJogada == null) {
            ...
            int posicaoCarta = forcaCartas
                .indexWhere((carta) => carta.valor == cartaJogada.valor);
        ...
        if (jogadorVencedor != -1) {
          ...
          if (equipe1 == 2 || equipe2 == 2) {
            ...
            equipe1 = 0;
            equipe2 = 0;
            jogadorAtual = baralho.listaJogador
                .firstWhere((jogador) => jogador.equipe == jogadorVencedor);
            CartasNaMesa();
            return;
          }
  ...
  List<CartaNaMesa> verificarCartasIguais(
      List<Cartas> forcaCartas, List<CartaNaMesa> cartasNaMesa) {
  ...
  List<Cartas> retornarListaDeForca() {
    ...
    return [
      cartas
    ]; // Assuming ajustarForcaCartas modifies cartas in place and returns it
  }
